describe("null_cornblath", {
  it("returns null_distribution with correct dims", {
    ...
    result <- null_cornblath(data, coords, parcellation, n_perm = 5L, seed = 1)
  ...
  it("errors for wrong data length", {
    ...
    expect_error(
      null_cornblath(1:5, coords, parcellation, n_perm = 2L),
      "must match"
    )
  })

  it("null values come from original data", {
    set.seed(42)
    n_lh <- 10
    n_rh <- 10
    coords <- list(
